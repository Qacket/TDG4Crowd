...
for i in range(8):
    mean = pd.read_csv('../../datasets/labelme_2/KS/KS_stat_mean_' + str(i) + '.csv')
    std = pd.read_csv('../../datasets/labelme_2/KS/KS_stat_std_' + str(i) + '.csv')


    path = '../../result/labelme_exp2/labelme_KS_exp2/exp2_label_' + str(i) + '.pdf'

    plt.title('exp2_label_' + str(i))
    y0 = list(mean['0'])
    y1 = list(mean['1'])
    y2 = list(mean['2'])
    y3 = list(mean['3'])

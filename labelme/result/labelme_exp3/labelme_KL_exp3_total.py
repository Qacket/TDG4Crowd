...
for i in range(8):
    ...
    mean_sub = pd.read_csv('../../datasets/labelme_3/KL/KL_mean_' + str(i) + '.csv')
    std_sub = pd.read_csv('../../datasets/labelme_3/KL/KL_std_' + str(i) + '.csv')
    if i == 0:
...
path = '../../result/labelme_exp3/labelme_KL_exp3/exp3_KL.pdf'
...
plt.errorbar(x, y5, yerr=ye5, color='black', capsize=3, linewidth=1, marker='v', mec='black', mfc='black', ms=7,
             label=u'DGCrowd')
leg = plt.legend(prop={'size': 8}, frameon=False)
plt.xticks(x, names)
plt.yscale('log')

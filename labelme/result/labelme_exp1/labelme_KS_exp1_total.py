...
for i in range(8):
    ...
    mean_sub = pd.read_csv('../../datasets/labelme_1/KS/KS_stat_mean_' + str(i) + '.csv')
    std_sub = pd.read_csv('../../datasets/labelme_1/KS/KS_stat_std_' + str(i) + '.csv')
    if i == 0:
...
path = '../../result/labelme_exp1/labelme_KS_exp1/exp1_KS.pdf'
y0 = list(mean['0'])
y1 = list(mean['1'])
y2 = list(mean['2'])
y3 = list(mean['3'])

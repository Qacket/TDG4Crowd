            ...
            if example in exist_task:
                ...
                for label_item in self.label_set:
                    if label_item == e2truth[example]:
                        l2p[label_item] = self.sigmoid(self.alpha[worker] * self.expbeta(self.beta[example]))
                    else:
                        l2p[label_item] = (1 - self.sigmoid(self.alpha[worker] * self.expbeta(self.beta[example]))) / (
                                    len(self.label_set) - 1)

...
for number in data_scale_range:
    ...
    mean = pd.read_csv('../../datasets/domain_relation_2/' + str(number) + '/mean.csv')
    std = pd.read_csv('../../datasets/domain_relation_2/' + str(number) + '/std.csv')
    path = '../../result/domain_relation_exp2/domain_relation_converge_exp2/exp2_converge_' + str(number) + '.pdf'
    ...
    plt.xlabel(str(number) + " annotators")  # Y轴标签
    plt.ylabel("ACC") #Y轴标签

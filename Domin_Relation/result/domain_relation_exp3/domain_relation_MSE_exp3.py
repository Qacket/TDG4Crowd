...
for number in data_scale_range:
    ...
    real_sort = np.argsort(pd.read_csv('../../datasets/domain_relation_3/' + str(number) + '/mean.csv').values[0])
    print(real_sort)
    for generate_method_name in generate_methods:

        acc = pd.read_csv('../../datasets/domain_relation_3/' + str(number) + '/' + generate_method_name + '/accuracy.csv').values
...
# mse = []
# for number in data_scale_range:
#     mean = pd.read_csv('../../datasets/domain_relation_3/' + str(number) + '/mean.csv').values
#
